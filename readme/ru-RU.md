...
> Совет: Выполните `bun run setup:ffmpeg` для автоматической загрузки FFmpeg/FFprobe и `bun run setup:upscaler` для Real-ESRGAN (AI-апскейлинг). Используйте `--force` для обновления.
