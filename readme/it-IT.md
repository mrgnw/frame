...
> Suggerimento: Esegui `bun run setup:ffmpeg` per scaricare automaticamente FFmpeg/FFprobe e `bun run setup:upscaler` per Real-ESRGAN (upscaling AI). Usa `--force` per aggiornare.
