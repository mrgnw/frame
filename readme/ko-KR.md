...
- **AI 업스케일링:** 고품질 비디오 업스케일링(x2, x4)을 위한 `Real-ESRGAN` 통합
...
> 팁: `bun run setup:ffmpeg` (또는 `npm run setup:ffmpeg`)로 FFmpeg/FFprobe를, `bun run setup:upscaler` (또는 `npm run setup:upscaler`)로 Real-ESRGAN(AI 업스케일링)을 자동 다운로드할 수 있습니다. `--force`로 기존 다운로드를 갱신할 수 있습니다.
